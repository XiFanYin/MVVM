...
abstract class BaseAdapter<DATA> : RecyclerView.Adapter<CommonViewHolder> {
    ...
    override fun onBindViewHolder(holder: CommonViewHolder, position: Int) {
        ...
        /*设置条目点击事件*/
        clickValue?.let { holder.itemView.setOnClickListener { view -> if (itemCount>adjPosition){it(view, adjPosition,mDatas!!.get(adjPosition))} } }
        /*设置条目的长按事件*/
        clickLongValue?.let { holder.itemView.setOnLongClickListener { view -> if (itemCount>adjPosition){it(view, adjPosition,mDatas!!.get(adjPosition))}; true } }
        /*封装子View的点击事件*/
        chindClickValue?.let {
            clickViewIds.forEach {
                holder.getView<View>(it)
                        .setOnClickListener { view -> chindClickValue?.let { it1 -> if (itemCount>adjPosition){ it1(it, view, adjPosition,mDatas!!.get(adjPosition)) } } }
            }
                    ...
                    (mRecyclerView.layoutManager as GridLayoutManager).spanSizeLookup =
                            object : GridLayoutManager.SpanSizeLookup() {
                                override fun getSpanSize(position: Int): Int {
                                    if (isHead(position) || isFood(position)||isEmptyViewShow()) {
